...
const VALID_INTERVALS = ['1h', '4h', '1d', '1w'];
const INTERVAL_LABELS = { '1h': 'ساعة', '4h': '٤ ساعات', '1d': 'يومي', '1w': 'أسبوعي' };

async function getTechnicalAnalysis(symbol, interval = '1d') {
  const s = symbol.toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (!s || s.length < 5) throw new Error('رمز غير صالح. مثال: BTCUSDT');
  const iv = VALID_INTERVALS.includes(interval) ? interval : '1d';

  let base = API, market = 'spot', klines;
  try { klines = await get(API, `/api/v3/klines?symbol=${s}&interval=${iv}&limit=250`); }
  catch (e) {
    if (!isBadSymbol(e)) throw e;
    base = FAPI; market = 'futures';
    klines = await get(FAPI, `/fapi/v1/klines?symbol=${s}&interval=${iv}&limit=250`);
  }
  ...
  const shortTimeframeWarning = (iv === '1h' || iv === '4h')
    ? `⚠️ إطار ${INTERVAL_LABELS[iv]} قصير — "الاتجاه العام" والمتوسط ٢٠٠ يعكسان مدى زمنياً أقصر بكثير مما يعنيانه عادة (${iv==='1h'?'يومان تقريباً':'نحو شهر'} بدل أشهر). فسّرهما كصورة قريبة المدى، لا بعيدة.`
    : null;

  return {
    symbol: s, market, interval: iv, intervalLabel: INTERVAL_LABELS[iv],
    shortTimeframeWarning,
