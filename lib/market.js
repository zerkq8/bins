...
/**
 * أدنى قاع وأعلى قمة خلال فترة معينة — خفيف الوزن (شموع فقط، بلا
 * تحليل أنماط). يُستخدم لتنبيهات الأسعار: هل وصل السعر لأدنى قاعه؟
 * "عند القاع" = ضمن ١٪ من أدنى سعر خلال كامل الفترة.
 */
async function getPeriodExtremes(symbol, days = 180) {
  ...
  let base = API, market = 'spot', klines;
  try {
    klines = await get(API, `/api/v3/klines?symbol=${s}&interval=1d&limit=${days}`);
  } catch (e) {
    if (!isBadSymbol(e)) throw e;
    base = FAPI; market = 'futures';
  ...
  return {
    symbol: s, market, days: klines.length,
    current, periodHigh, periodLow,
    ...
    atLow: distFromLowPct !== null && distFromLowPct <= 1,     // ضمن ١٪ من القاع
    atHigh: distFromHighPct !== null && distFromHighPct >= -1,  // ضمن ١٪ من القمة
  };
}

module.exports = { getSymbol, getHistoricalContext, getPeriodExtremes };
