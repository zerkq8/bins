 ...
 * - وقف الخسارة/هدف الربح = نسبة٪ من الهامش، مبنية على ATR الخاص
 *   بكل عملة، لكن بـ"حد أدنى مضمون" = القيمة المُدخلة في الإعدادات
 *   نفسها. يعني: النتيجة لا يمكن أبداً أن تكون أصغر من القيمة التي
 *   كتبها المستخدم (لا تكرار لمشكلة "ربح صغير جداً" السابقة)، لكنها
 *   تقدر تكبر على العملات شديدة التقلب لتستفيد من فكرة ATR الأصلية.
 *   حد أقصى أيضاً = 3× القيمة المُدخلة، لمنع أي توسّع غير معقول لو
 *   جاءت بيانات ATR شاذة.
 */
...
// مرجع ثابت لتحويل نسب الوقف/الهدف المُدخلة إلى مضاعِف يُطبَّق على
// ATR% الفعلي للعملة — لو ATR%_العملة = هذا الرقم بالضبط، فالمضاعف
// = 1.0 والنسبة المقترحة من ATR تطابق القيمة المُدخلة تماماً.
const ATR_REFERENCE_PCT = 20;
const ATR_MULTIPLIER_MIN = 0.5;
const ATR_MULTIPLIER_MAX = 3;
// حد التوسّع الأقصى فوق القيمة المُدخلة (٣× كحد أقصى مطلق للنتيجة
// النهائية، بصرف النظر عن ATR أو المضاعف).
 ...
 * "عدم توفر ATR"، لا خطأ يوقف الصفقة؛ عندها تُستخدم القيمة المُدخلة
 * كما هي (نفس سلوك الحد الأدنى، فلا فرق عملي على المستخدم).
...
/**
 * يحسب النسبة النهائية (وقف أو هدف) بمنطق "حد أدنى مضمون":
 * - لا يمكن أن تقل أبداً عن basePct (القيمة المُدخلة في الإعدادات)
 * - يمكن أن تكبر حتى basePct × MAX_EXPANSION_FACTOR كحد أقصى مطلق
 * - إن لم يتوفر ATR صالح، تُرجع basePct كما هي دون أي تعديل
 */
function computeFlooredPct(basePct, atrPct) {
  ...
  // الحد الأدنى = basePct دائماً (Math.max)، الحد الأقصى = maxAllowed (Math.min)
...
async function executeEntry({ symbol, side, settings }) {
  ...
  try {
    ...
    // وقف/هدف بمنطق "حد أدنى مضمون": لا يمكن أن يقلّا أبداً عن القيمة
    // المُدخلة في الإعدادات (settings.stopLossPct/takeProfitPct)، لكن
    // يمكن أن يكبرا على العملات شديدة التقلب (حتى 3× كحد أقصى مطلق).
    // عند أي تعذّر في جلب/حساب ATR، تُستخدم القيمة المُدخلة كما هي
    // تماماً — لا فرق عملي على المستخدم في هذه الحالة.
    let atrInfo = null;
