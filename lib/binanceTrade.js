...
/**
 * صافي الدخل (ربح/خسارة محقق + عمولات + تمويل) خلال آخر 24 ساعة —
 * هذا يعكس التغيّر الفعلي في رصيد المحفظة بسبب التداول، لا الربح
 * المحقق وحده. قراءة فقط.
 * ⚠️ محدود بحد أقصى 1000 سجل دخل (حد باينس لهذه النقطة) — كافٍ لبوت
 * عادي، لكن نشاط استثنائي الكثافة خلال 24 ساعة قد يتجاوزه.
 */
async function get24hIncome() {
  const startTime = Date.now() - 24 * 60 * 60 * 1000;
  const income = await signedRequest('GET', '/fapi/v1/income', { startTime, limit: 1000 });
  const rows = Array.isArray(income) ? income : [];
  const pnl = rows.reduce((sum, r) => sum + (parseFloat(r.income) || 0), 0);
  return { pnl, count: rows.length, startTime };
}

module.exports = { ping, getAccountInfo, getPositions, get24hIncome, signedRequest, publicRequest, MODE };
