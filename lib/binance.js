...
async function getTop() {
  const data = await call(ENDPOINTS.list, { body: listBody(1, 20, '90D') });
...
async function getTrader(id) {
  const [detail, closed, open] = await Promise.allSettled([
    ...
    call(ENDPOINTS.closedTrades, { body: { portfolioId: id, pageNumber: 1, pageSize: 20 } }),
...
/**
 * يمسح صفحات القائمة بالتوازي (لا تسلسلياً كـ getRanked) ويجمّع كل
 * المتداولين بلا أي فلترة أو ترتيب — تجميع خام فقط مع إزالة التكرار
 * بالمعرّف. يُستخدم في الاختيار اليومي لـ"توافق النخبة"، حيث لا حاجة
 * لترتيب الصفحات ولا لإيقاف المسح مبكراً، وسرعة الجمع أهم من ترتيبه.
 */
async function getEliteCandidates({ pages = 6, pageSize = 50 } = {}) {
  const results = await Promise.allSettled(
    Array.from({ length: pages }, (_, i) => call(ENDPOINTS.list, { body: listBody(i + 1, pageSize) }))
  );
  const seen = new Set();
  const all = [];
  for (const r of results) {
    if (r.status !== 'fulfilled') continue;
    for (const raw of listOf(r.value)) {
      const t = normTrader(raw);
...
/**
 * آخر الصفقات المغلقة لمتداول، مطبَّعة (pnl بالدولار الفعلي من باينس، roi٪،
 * closeTime) ومرتّبة من الأحدث. من يُخفي سجله تُرجع له باينس قائمة فارغة.
 */
async function getClosedTrades(id, pageSize = 50) {
  const data = await call(ENDPOINTS.closedTrades, { body: { portfolioId: id, pageNumber: 1, pageSize } });
  return listOf(data).map(normClosed).sort((a, b) => (b.closeTime || 0) - (a.closeTime || 0));
}

/**
 * ربح/خسارة الناسخين الإجمالي (copierPnl) لمتداول واحد، عبر نقطة التفاصيل
 * مباشرة — لا تحتاج تصفّح صفحات. ⚠️ نقطة التفاصيل هذه لا تعيد mdd/roi/pnl/
 * winRate إطلاقاً (تحقّقنا حياً من رد باينس الفعلي) — تلك متوفرة فقط من
 * نقطة القائمة المُرتّبة (getTop/getRanked)، ولا يوجد فيها بحث بمعرّف مباشر.
 */
async function getTraderCopierPnl(id) {
  const data = await call(ENDPOINTS.detail, { id });
  return num(data?.copierPnl);
}

module.exports = {
  getTop, searchTraders, getTrader, getSignals, getRanked, getWatch, getEliteCandidates,
  getPositionStatus, getPositionStatusBulk, getClosedTrades, getTraderCopierPnl, jsonHandler,
