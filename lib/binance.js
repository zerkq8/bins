...
function normOpen(t = {}) {
  ...
  // roi تقديري: هامش ≈ |الكمية × سعر الدخول| ÷ الرافعة
  let roi = null;
  if (amount !== null && entryPrice !== null && leverage && pnl !== null) {
    const margin = Math.abs(amount * entryPrice) / leverage;
    if (margin > 0) roi = (pnl / margin) * 100;
  }
...
async function getTrader(id) {
  ...
  const closedList = listOf(val(closed)).map(normClosed)
    ...
    .slice(0, 10);
  const openList = listOf(val(open)).map(normOpen).filter(isOpenPosition);
...
module.exports = {
  getTop, searchTraders, getTrader, getSignals, getRanked, getWatch, jsonHandler,
