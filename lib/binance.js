...
async function getRanked({ mode = 'trust', scan = 50 } = {}) {
  ...
  // الحد الأدنى للقبول — يختلف بحسب التبويب
  const BAR = mode === 'trust'
    ? { age: 180, mdd: 25, lev: null, label: 'سجل ٦ أشهر فأكثر · تراجع تحت ٢٥٪ · ناسخون رابحون' }
    : { age: 90,  mdd: 40, lev: null, label: 'سجل ٣ أشهر فأكثر · تراجع تحت ٤٠٪ · ناسخون رابحون' };
  ...
  for (const t of all) {
    ...
    // نتيجة الناسخين شرط أساسي في التبويبين
    ...
    if (mode === 'trust') {
      ...
      why = [
        `سجل ${age} يوماً`,
        `تراجع ${mdd.toFixed(0)}%`,
        `ناسخون ${t.copierPnl > 0 ? 'رابحون' : '—'}`,
      ];
    } else {
      // العائد لكل وحدة مخاطرة — الرقم الخام وحده مضلل
      const adjusted = (t.roi || 0) / mdd;
      score = adjusted + Math.min(age / 30, 12) + fill * 10;
      why = [
        `عائد/مخاطرة ${adjusted.toFixed(1)}`,
        `سجل ${age} يوماً`,
        `تراجع ${mdd.toFixed(0)}%`,
      ];
    }

    passed.push({ ...t, age, score: Math.round(score * 10) / 10, why, fill });
  }
