...
لا صفقات مغلقة اليوم لأي متداول متابَع.`;
  ...
  مراكز مفتوحة الآن: ${t.openNow === null ? '—' : t.openNow}`;
  });
...
async function runDailyReport({ deviceKey }) {
  ...
  // مسح كامل لسجل باينس الفعلي لكل متداول في النطاق — لا فقط من ظهر في
  // السجل التقريبي — لالتقاط أي صفقة فُتحت وأُغلقت بين دورتي فحص متتاليتين
  // قبل أن تُسجَّل أصلاً. مهلة أضيق (DAILY_HIST_TIMEOUT_MS) لأن العدد قد
  // يصل حتى ٢٠ طلباً متوازياً.
  const { getClosedTrades } = require('./binance');
  const hist = await Promise.allSettled(
    scanIds.map((id) => withTimeout(getClosedTrades(id, 50), DAILY_HIST_TIMEOUT_MS))
  );

  const traders = scanIds.map((id, i) => {
    let trades = null, source = 'estimate';
    if (hist[i].status === 'fulfilled') {
      const inWindow = hist[i].value.filter((t) => t.closeTime >= day.startMs && t.closeTime <= day.endMs);
    ...
    return { id, name: nameOf[id] || id, count: trades.length, wins, losses, pnlTotal, source, openNow: id in openMap ? openMap[id] : null };
  }).filter(Boolean);
...
module.exports = {
  ...
  __internal: { kuwaitDayBounds, dailyReportMsg, closedLogRow, hasReportBeenSentToday, claimDailyReportSlot },
};
